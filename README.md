...
A local-first workspace for bug bounty recon. Browse programs on six platforms, import their scope, run a 27-tool pipeline against it, triage the findings, and export a report — all on your machine, in one SQLite database, with no cloud and no telemetry.
...
**Pipeline configuration — nine stages, twenty-seven tools, per-tool arguments, tool availability detected on your machine**
...
| Pipeline | 9 stages, 27 tools, per-tool arguments, concurrency settings — and **real run controls**: Pause waits between tools, Resume continues, Cancel kills the in-flight subprocess and finalizes the run honestly (current stage `cancelled`, remaining stages `skipped`) |
| Tool detection | The app probes your actual `$PATH` at startup and tells you which of the 27 tools are installed, with versions — no fake status lights |
...
### Installing the 27 pipeline tools (optional but recommended)

The app runs fine without them — it will honestly show `0/27 tools installed` and let you configure everything — but the pipeline only executes tools that exist on your machine.

On Linux, the bundled installer handles package-name differences across distros (Debian/Ubuntu, Arch, Fedora, openSUSE, Alpine, Void, Gentoo), installs the Go, Python and Ruby toolchains, builds the Go tools, installs the Ruby-based CMS scanners (`whatweb`, `wpscan`), clones SecLists, writes a resolver list, and downloads nuclei templates:
...
The vulnerability stage layers seven complementary scanners: **nuclei** for template/CVE matching, **nikto** for web-server misconfigurations and outdated software, **dalfox** for parameter-level XSS with payload evidence, **tlsx** for TLS configuration audits (deprecated protocol versions, weak ciphers, certificate problems), **cariddi** for crawl-time exposure hunting (leaked secrets such as AWS keys, juicy parameters, verbose errors, exposed `.env`/`.sql`/backup files), **whatweb** for CMS and technology fingerprinting (WordPress, Drupal, Joomla, servers, frameworks with versions), and **wpscan** for dedicated WordPress scanning (core version and exposure status, main theme and plugin versions with outdated checks, `debug.log` and `readme.html` exposure, upload directory listings, XML-RPC). Each produces its own finding types with severity, evidence, and remediation — and a scanner that exits with errors mid-scan still has its partial output parsed, with an honest warning in the run log.
...
- **HackerOne (optional API key)** — create an API token on hackerone.com (Settings → API Tokens) and save it under Settings → Platform Accounts to also load programs visible only to your account and fetch scope through the official Hacker API (`api.hackerone.com`). Public data works without it.
- **Bugcrowd** — program scope needs API credentials. Create them on bugcrowd.com (profile menu → API Credentials) and paste the token username + password under Settings → Platform Accounts; they are validated against the official Bugcrowd API before being saved. Without them you still get the full public program listing — scope import will tell you exactly what's missing instead of showing a spinner forever.
- **Intigriti** — all program data is behind the authenticated Researcher API. Create a Personal Access Token on app.intigriti.com (profile → Personal Access Tokens) and save it under Settings → Platform Accounts; it is validated live before being stored only in your local database.
- **YesWeHack / Immunefi** — fully public sources; these platforms offer no researcher API keys (YesWeHack PATs are issued only to Program Manager / Business Unit roles), so there is nothing to connect.
...
- The Bugcrowd/Intigriti integrations use the platforms' official documented APIs (api.bugcrowd.com JSON API, api.intigriti.com Researcher API) and follow their published schemas — they can still break if those platforms change their APIs.
